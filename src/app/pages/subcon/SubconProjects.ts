...
@Component({
  ...
  templateUrl: './SubconProjects.html'
})
export class SubconProjects {
  ...
  cache: ICache = {
    key: this.app.auth.user.username + '-subcon-projects',
    group_key: 'api_data',
    ttl: 60 * 60 * 24
  };
  data: any;
  ...
  async getData(refresh?) {
    ...
    const res = await this.app.cache.getOrSetItem(this.cache.key,
      () => this.ws.getAPI('apiService/getSubconProject', 
        {
          id: this.app.auth.user.userid
        }
      ).then((res: any) => {
        return res;
  ...
  goToDailyReport(id) {
    this.navCtrl.navigateForward('subcon-daily-report/' + id);
  }
