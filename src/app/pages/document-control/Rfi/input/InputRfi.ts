...
@Component({
  selector: "Rfi/",
  templateUrl: "./InputRfi.html",
  styleUrls: ["./InputRfi.scss", "../../Global.scss"],
})
export class InputRfi implements OnInit {
  @ViewChild("scroll") scroller: VirtualScrollerComponent;
  props: IProps = {
    title: "Input Document RFI",
    isRoot: false,
    stepper: {
      idActive: 1,
  ...
  async selectProyek(ev) {
    ...
    if (ev.value.id) {
      ...
      this.userItemsList = get(userItems, "data");
    }
  }
  ...
  async ionViewDidEnter() {
    ...
    this.filter.data.name = this.filter.list[0].name;
    this.filter.data.id = this.filter.list[0].id;
  }

  ngOnInit() {
    this.onRfiForm = this.formBuilder.group({
  ...
  async getData(refresh?, choice?) {
    ...
    const data = await this.ws.getAPI("/apiService/FormRfi", {});
