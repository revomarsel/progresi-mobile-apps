...
@Component({
  selector: 'app-subcon-projects',
  templateUrl: './MaterialsPlanning.html'
})
export class MaterialsPlanning {
  ...
  cache: ICache = {
    key: this.app.auth.user.username + '-materials-planning',
    group_key: 'api_data',
    ttl: 60 * 60 * 24
  };
  ...
  dataView: any;
  grandTotal: number = 0;
  groupingList: any = ['task', 'material', 'date'];
  minDateRange: string;
  maxDateRange: string;
  detail: any;
  ...
  endScrollFunc: any = () => {
    ...
    this.data = this.data.slice(0, this.dataViewIncrement);
    this.searchValue();
  ...
  async getData(refresh?) {
    ...
    const res = await this.app.cache.getOrSetItem(this.cache.key,
      () => this.ws.getAPI('/apiService/GetMaterialRecap',
        {
          pid: this.pid,
          bid: this.bid
        }
    ...
    if (this.searchKey.trim() !== '') {
      this.data = this.defaultItems.filter((item) => {
        if (option === 'task') {
          ...
          return item.material.toLowerCase().indexOf(this.searchKey.toLowerCase()) > -1;
        }
      });
    } else {
    ...
    this.data = this.defaultItems.filter((item) => {
      if (this.minDateRange) {
        if (this.maxDateRange) {
          return this.alpro.checkIfDateIsBetween(item.date, this.minDateRange, this.maxDateRange);
        } else {
          return this.alpro.dateIsGreaterThanEquals(item.date, this.minDateRange);
        }
      } else if (this.maxDateRange) {
        return !this.alpro.dateIsLessThanEquals(item.date, this.maxDateRange);
      } else return true;
    });
    this.changeViewGrouping(this.groupByOption);
  }
  ...
  changeViewGrouping(option?) {
    ...
    this.dataView.map((val, idx) => {
      ...
      let materialUnit = '';
      val.items.map(item => {
        totalVolume += Number(item.qty);
        totalCost += Number(item.price) * Number(item.qty);
        materialPrice = Number(item.price);
        materialUnit = item.unit;
      });
      this.dataView[idx].totalCost = totalCost;
      this.dataView[idx].totalVolume = totalVolume;
      this.dataView[idx].materialPrice = materialPrice;
      this.dataView[idx].materialUnit = materialUnit;
