...
@Component({
  ...
  templateUrl: './EquipmentsPlanning.html'
})
export class EquipmentsPlanning {
  ...
  cache: ICache = {
    key: this.app.auth.user.username + '-equipments-planning',
    group_key: 'api_data',
    ttl: 60 * 60 * 24
  };
  data: any;
  ...
  async getData(refresh?) {
    ...
    const res = await this.app.cache.getOrSetItem(this.cache.key,
      () => this.ws.getAPI('/apiService/GetEquipmentRecap',
        {
          pid: this.pid,
          bid: this.bid
        }
  ...
  changeViewGrouping(option?) {
    ...
    this.dataView.map((val, idx) => {
      ...
      val.items.map(item => {
        ...
        equipmentPrice = Number(item.price);
        equipmentUnit = item.unit;
      });
      this.dataView[idx].totalCost = totalCost;
      this.dataView[idx].totalVolume = totalVolume;
      this.dataView[idx].equipmentPrice = equipmentPrice;
      this.dataView[idx].equipmentUnit = equipmentUnit;
