...
@Component({
  selector: "InputEquipmentDemob",
  templateUrl: "./InputEquipmentDemob.html",
})
export class InputEquipmentDemob {
  ...
  async getData(refresh?) {
    const data = await this.ws.getAPI("apiService/getEquipmentDemobInput", {
      pid: this.projectId,
      date: this.date,
    });
    ...
    const itemsList = await this.ws.getAPI("apiService/getEquipmentDemobList", {
      pid: this.projectId,
      date: this.date,
    });
  ...
  async saveData() {
    ...
    const res = await this.ws.postAPI('apiService/PostEquipmentDemob', payload);
    if (res) {
      alert('Post Success');
      this.exitForm();
    } else {
  ...
  async uploadImage() {
    const modal = await this.modalCtrl.create({
      ...
      componentProps: { data: this.medias, isViewMode: this.isViewMode, title: 'Demob. Peralatan', date: this.date }
    });
