...
@Component({
  selector: 'ProjectPlanners',
  templateUrl: './ProjectPlanners.html'
})
export class ProjectPlanners {
  ...
  cache: ICache = {
    key: this.app.auth.user.username + '-project-planners',
    group_key: 'api_data',
    ttl: 1000
  };
  auth: any;
  ...
  async getData(refresh?) {
    ...
    this.data = await this.app.cache.getOrSetItem(`${this.cache.key}-${this.projectId}`,
      () => this.ws.getAPI('/apiService/getProjectPlanners', {
        id: this.projectId
      }).then((res: any) => {
        return res;
